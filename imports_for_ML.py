...
def plot_decision(X, y, clf=None, cm=None):
    '''
    Plot decision function of a given classifier. (updated)
    ...
    if clf is not None:
        ...
        plt.contourf(xx, yy, Z, levels=[0, 0.5, 1], cmap=cm, alpha=0.8)
        plt.contour(xx, yy, Z, levels=[0.5], colors='k')

    for cls in np.unique(y):
        this_class = y == cls
        plt.scatter(X[this_class, 0], X[this_class, 1], edgecolor='k')

    if clf is not None:
        pred = clf.predict(X)
        if hasattr(clf, 'predict_classes'):
            pred = clf.predict_classes(X).ravel()
        else:
            pred = (pred.ravel() > 0.5).astype('int')
        corr = (pred == y).mean()
        plt.title('Correctness = {}'.format(corr))
